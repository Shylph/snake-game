# refer: https://raw.githubusercontent.com/golbin/TensorFlow-Tutorials/master/12%20-%20DQN/model.py
...
class DQN:
    ...
    def __init__(self, session, width, height, n_action):
        ...
        self.input_X = tf.placeholder(tf.float32, [None, width, height, self.STATE_LEN], name='x')
        # 각각의 상태를 만들어낸 액션의 값들입니다. 0, 1, 2 ..
        self.input_A = tf.placeholder(tf.int32, [None], name='a')
    ...
    def _build_network(self, name):
        ...
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, filters=16, kernel_size=(8, 8), strides=(4, 4),
            ...
            Q = tf.layers.dense(model, self.n_action)
        return Q
    ...
    def train(self):
        ...
        next_Q = self.session.run(self.target_Q,
        ...
        for i in range(self.BATCH_SIZE):
            if terminal[i]:
                Y.append(float(reward[i]))
            else:
                Y.append(float(reward[i]) + self.GAMMA * np.max(next_Q[i]))
